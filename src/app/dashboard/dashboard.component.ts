...
@Component({
  ...
  imports: [
    ...
    MatProgressSpinnerModule, MatCardModule, MatChipsModule, MatDividerModule,
  ],
  template: `
    @if (isAdmin()) {
      <!-- ════════════════════════════════════════════════════════════════════ -->
        ...
        @if (loading()) {
          ...
          <div class="row-two">
            ...
            <div class="roles-card">
              <div class="card-header"><h2>Répartition des rôles</h2></div>
          ...
          <section class="availability-section">
            <div class="card-header"><h2>Disponibilité du fonds</h2></div>
            <div class="avail-bars">
              ...
              <div class="avail-item">
                ...
                <span class="avail-pct">{{ stats.activeLoans > 0 ? ((stats.overdueLoans / stats.activeLoans) * 100 | number:'1.0-0') : 0 }}%</span>
              </div>
              <div class="avail-item">
      ...
      <!-- ════════════════════════════════════════════════════════════════════ -->
      <!-- DASHBOARD LECTEUR / BIBLIOTHÉCAIRE                                  -->
      <!-- ════════════════════════════════════════════════════════════════════ -->
      <section class="dashboard">
        ...
        <section class="content-grid">
          <mat-card class="panel panel-wide">
            ...
            <div class="loan-list">
              @for (loan of recentLoans; track loan.title) {
                <article class="loan-item">
                  ...
                  <div class="loan-copy">
                    ...
                    <p>{{ loan.author }}</p>
                    <div class="loan-meta">
                      <span>Retour prévu : {{ loan.due }}</span>
                      <span [class.late]="loan.statusTone === 'late'">{{ loan.status }}</span>
                    </div>
                  </div>
                </article>
          ...
          <mat-card class="panel">
            ...
            <div class="reservation-list">
              @for (reservation of reservations; track reservation.title) {
                <article class="reservation-item">
                  ...
                  <div class="reservation-footer">
                    ...
                    <div class="chip-row">
                      @for (chip of reservation.chips; track chip) {
  ...
  styles: [`
    /* ════ ADMIN DASHBOARD ════════════════════════════════════════════════════ */
    .admin-dashboard { display:flex; flex-direction:column; gap:1.5rem; max-width:1400px; margin:0 auto; }
    .dash-header { display:flex; align-items:flex-start; justify-content:space-between; gap:1rem; }
    .dash-header h1 { margin:0; font-family:'DM Serif Display',Georgia,serif; font-size:1.7rem; }
    .dash-subtitle { margin:.25rem 0 0; color:var(--bh-ink-light); font-size:.9rem; }
    .loading-overlay { display:flex; flex-direction:column; align-items:center; gap:1rem; padding:5rem; color:var(--bh-ink-light); }

    .kpi-grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(210px,1fr)); gap:1rem; }
    .kpi-card { border-radius:20px; padding:1.4rem; display:flex; align-items:center; gap:1rem; transition:transform .15s,box-shadow .2s; }
    ...
    .kpi-icon-wrap { width:3rem; height:3rem; border-radius:12px; background:rgba(255,255,255,.2); display:grid; place-items:center; flex-shrink:0; }
    ...
    .chart-card,.roles-card,.topbooks-card,.activity-card,.availability-section {
      ...
      border:1px solid rgba(0,0,0,.07); box-shadow:0 2px 8px rgba(0,0,0,.04);
    }
    .card-header { display:flex; align-items:center; justify-content:space-between; margin-bottom:1.25rem; gap:.5rem; }
    .card-header h2 { margin:0; font-family:'DM Serif Display',Georgia,serif; font-size:1.15rem; }
    .card-subtitle { font-size:.82rem; color:var(--bh-ink-light); }
    .see-all { font-size:.85rem; color:var(--bh-forest); }

    .row-two { display:grid; grid-template-columns:1fr 320px; gap:1.5rem; }
    @media (max-width:900px) { .row-two { grid-template-columns:1fr; } }

    .bar-chart { display:flex; align-items:flex-end; gap:.6rem; height:140px; padding-bottom:.5rem; }
    .month-group { display:flex; flex-direction:column; align-items:center; gap:.35rem; flex:1; }
    .bars { display:flex; align-items:flex-end; gap:2px; }
    .bar { width:14px; border-radius:4px 4px 0 0; display:flex; align-items:flex-start; justify-content:center; position:relative; min-height:6px; transition:opacity .2s; }
    .bar:hover { opacity:.75; }
    .bar-val { font-size:.6rem; color:#fff; position:absolute; top:3px; writing-mode:vertical-rl; transform:rotate(180deg); opacity:0; transition:opacity .2s; }
    ...
    .donut-ring { fill:none; stroke-width:18; stroke-linecap:round; transition:stroke-dashoffset .4s ease; }
    .donut-center-val { font-size:22px; font-weight:800; fill:var(--bh-ink); transform:rotate(90deg); transform-origin:center; }
    .donut-center-label { font-size:10px; fill:var(--bh-ink-light); transform:rotate(90deg); transform-origin:center; }
    ...
    .row-three { display:grid; grid-template-columns:1fr 1fr; gap:1.5rem; }
    @media (max-width:800px) { .row-three { grid-template-columns:1fr; } }

    .topbooks-list { display:flex; flex-direction:column; gap:.6rem; }
    .topbook-row { display:flex; align-items:center; gap:.75rem; padding:.5rem .6rem; border-radius:10px; transition:background .15s; }
    .topbook-row:hover { background:rgba(0,0,0,.03); }
    .book-rank { width:1.6rem; height:1.6rem; border-radius:50%; background:#f1f5f9; color:var(--bh-ink-light); display:grid; place-items:center; font-size:.78rem; font-weight:700; flex-shrink:0; }
    ...
    .activity-icon { width:2.2rem; height:2.2rem; border-radius:10px; display:grid; place-items:center; flex-shrink:0; }
    ...
    .avail-track { flex:1; height:10px; background:#f1f5f9; border-radius:5px; overflow:hidden; }
    .avail-fill { height:100%; border-radius:5px; transition:width .5s ease; }
    ...
    .dashboard-hero { display: flex; align-items: flex-start; justify-content: space-between; gap: 1rem; flex-wrap: wrap; }
    .dashboard-hero h1 { margin: 0; font-family: 'DM Serif Display', Georgia, serif; font-size: 1.7rem; }
    .eyebrow { margin: 0 0 .25rem; font-size: .8rem; text-transform: uppercase; letter-spacing: .08em; color: var(--bh-forest); font-weight: 600; }
    .lead { margin: .4rem 0 0; color: var(--bh-ink-light); }
    .hero-actions { display: flex; gap: .75rem; flex-wrap: wrap; }
    .stats-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem; }
    .stat-card { display: flex; align-items: center; gap: 1rem; padding: 1.25rem !important; border-radius: 16px !important; }
    .stat-card.tone-forest .stat-icon { color: var(--bh-forest); }
    .stat-card.tone-amber  .stat-icon { color: var(--bh-amber); }
    .stat-card.tone-warn   .stat-icon { color: #ef4444; }
    .stat-icon mat-icon { font-size: 2rem; width: 2rem; height: 2rem; }
    .stat-card p { margin: 0; font-size: .85rem; color: var(--bh-ink-light); }
    .stat-card strong { font-size: 2rem; font-weight: 800; line-height: 1; }
    .stat-card span { font-size: .8rem; color: var(--bh-ink-light); }
    .content-grid { display: grid; grid-template-columns: 1fr 380px; gap: 1.5rem; }
    @media (max-width: 900px) { .content-grid { grid-template-columns: 1fr; } }
    .panel { border-radius: 20px !important; overflow: hidden; }
    .panel-head { display: flex; align-items: center; justify-content: space-between; padding: 1.25rem 1.5rem; border-bottom: 1px solid rgba(0,0,0,.06); }
    .section-title { margin: 0; font-weight: 700; font-size: .95rem; }
    .section-subtitle { font-size: .8rem; color: var(--bh-ink-light); margin: .1rem 0 0; display: block; }
    .loan-list { display: flex; flex-direction: column; }
    .loan-item { display: flex; align-items: center; gap: 1rem; padding: 1rem 1.5rem; }
    .book-cover { width: 2.5rem; height: 3.2rem; border-radius: 6px; background: var(--bh-forest-pale); display: grid; place-items: center; flex-shrink: 0; }
    .book-cover mat-icon { color: var(--bh-forest); font-size: 1.3rem; }
    .loan-copy h3 { margin: 0 0 .2rem; font-size: .95rem; font-weight: 600; }
    .loan-copy p { margin: 0 0 .35rem; font-size: .82rem; color: var(--bh-ink-light); }
    .loan-meta { display: flex; gap: .75rem; font-size: .8rem; color: var(--bh-ink-light); }
    .late { color: #ef4444 !important; font-weight: 700; }
    .reservation-list { display: flex; flex-direction: column; gap: .5rem; padding: 1rem 1.5rem; }
    .reservation-item { border-radius: 12px; padding: 1rem; background: rgba(0,0,0,.02); }
    .reservation-item h3 { margin: 0; font-size: .9rem; font-weight: 600; }
    .reservation-item p { margin: .15rem 0 .5rem; font-size: .82rem; color: var(--bh-ink-light); }
    .reservation-footer strong { display: block; font-size: .82rem; margin-bottom: .4rem; }
